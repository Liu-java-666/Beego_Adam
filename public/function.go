...
//根据日期获取月份
func GetMonthByDate(date []uint8) string{
	str := string(date)
	//字符串转时间
...
//根据日期获取当月天
func GetDayByDate(date []uint8)string{
	str := string(date)
	//字符串转时间
	tm, _ := time.ParseInLocation("2006-01-02 15:04:05", str, time.Local)
	//格式化
...
func GetDateFmt(date []uint8)string{
	str := string(date)
	//字符串转时间
	tm, _ := time.ParseInLocation("2006-01-02 15:04:05", str, time.Local)
	//格式化
...
func GetIMUserID(account string) int {
	s := strings.Replace(account, beego.AppConfig.String("Pre"), "", 1)
	userid, _ := strconv.Atoi(s)
	return userid
}

//时间戳格式化
func DateFmt(tmstp int) string{
	t := int64(tmstp)
	tm := time.Unix(t, 0)
	now := time.Now()
...
//日期格式化
func DateShortFmt(dt []uint8) string{
	str := string(dt)
	//字符串转时间
	tm, _ := time.ParseInLocation("2006-01-02 15:04:05", str, time.Local)
	//格式化
...
//时间戳格式化 16:16
func TimeFmt(tmstp int) string{
	t := int64(tmstp)
	timeTemplate := "15:04"
	tm := time.Unix(t, 0).Format(timeTemplate)
	fmt.Println(tm)
	return tm
}